...
export const dbSqlitePath = path.resolve(process.cwd(), "Daltoon_Bot.db");
...
export function readSqliteDb(forceFresh: boolean = false): DbSchema {
  ...
  try {
    ...
    const hasValidToken =
      ...
      currentSettings.botToken !== "DUMMY_TOKEN" &&
      String(currentSettings.botToken).trim() !== "";
    const hasValidOwner =
      currentSettings.ownerId && Number(currentSettings.ownerId) > 0;

    db.isNewInstall = !hasValidToken || !hasValidOwner || db.isNewInstall === true;
