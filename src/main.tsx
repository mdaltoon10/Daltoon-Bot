  ...
  window.fetch = async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
    ...
    if (url.startsWith("/api/") || url.includes("/api/")) {
      ...
      try {
        ...
        // Public endpoints that should never invalidate user login state
        const publicEndpoints = ["/api/login", "/api/system/check-update", "/api/system/version", "/api/health", "/api/vpn-plans", "/api/miniapp"];
        const isPublicEndpoint = publicEndpoints.some((ep) => url.includes(ep));

        // If a protected admin endpoint returns 401 unauthorized on custom domain
        if (
          response.status === 401 &&
          !isPublicEndpoint &&
          ...
          if (localStorage.getItem("daltoon_dashboard_auth") === "true") {
            console.warn("[Auth Interceptor] Admin session invalid or expired for endpoint:", url);
