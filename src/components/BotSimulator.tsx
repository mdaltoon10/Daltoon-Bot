...
export default function BotSimulator({
  ...
  const currentUser: User = simulatedUsers.find(u => u.userId === activeUserId) || users.find(u => u.userId === activeUserId) || users[0] || { userId: 6536288293, username: "GuestUser", walletBalance: 0, activePlansCount: 0, joinDate: new Date().toISOString(), status: "active", referralCount: 0, referralRewards: 0, referralRewardTotal: 0 };
    ...
    if (text === (settings?.btnTextBuyNew || t.btnBuyNew) || text.includes("خرید") || text.includes("Buy") || text.includes("Plan")) {
        ...
        if (activeServers.length > 0) {
          const inlineServers = activeServers.map((s: any) => {
            const pLabel = s.panelType === "rebecca" ? "Reebeka" : s.panelType === "pasarguard" ? "Pasarguard" : s.panelType === "dui" ? "D-UI" : "X-UI";
            return [
              { text: `🌐 [${pLabel}] ${s.name}`, action: `srvsel_${s.id}` }
            ];
          });
  ...
  const handleInlineClick = (action: string) => {
    ...
    if (action.startsWith("srvsel_")) {
      ...
      if (serverId && settings?.isCustomPricingActive !== false) {
        inlineCats.push([
