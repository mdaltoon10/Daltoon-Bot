...
export default function SettingsPanel({
  ...
  // Anti-Bot Captcha config state
  const [captchaActive, setCaptchaActive] = useState<boolean>(
    settings.captchaActive !== undefined
      ? settings.captchaActive
      : (settings.referralCaptchaActive !== false)
  );
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSaveSettings({
      ...
      mandatoryJoinText,
      captchaActive,
      referralCaptchaActive: captchaActive,
      autoBackupEnabled,
      autoBackupInterval,
      qrTemplate,
      qrColor,
  ...
  return (
    <div id="settings-tab" className="max-w-4xl mx-auto space-y-6">
      ...
      {/* Anti-Bot Captcha (Human Verification) Toggle */}
      <div className="bg-[#111827] border border-indigo-500/20 p-5 rounded-xl space-y-4 shadow-sm relative overflow-hidden group">
        <h3 className="font-display font-medium text-lg text-white flex items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            <ShieldCheck className="w-5 h-5 text-indigo-400" />
            <span>
              {translateText("Anti-Bot Captcha & Human Verification", "تست ضد ربات و کپچا (احراز هویت انسانی)", lang)}
            </span>
          </div>
          ...
          <button
            type="button"
            onClick={() => setCaptchaActive(!captchaActive)}
            className={`relative inline-flex h-5 w-10 shrink-0 cursor-pointer rounded-full transition-colors duration-300 focus:outline-none ${
              captchaActive
                ? "bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.35)]"
                : "bg-slate-800"
          ...
          >
            <div
              className="absolute flex items-center justify-center h-4 w-4 rounded-full bg-white transition-all duration-300 ease-in-out"
              style={{
                left: captchaActive ? "22px" : "2px",
                top: "2px",
                color: captchaActive ? "#059669" : "#94a3b8",
              }}
            >
              <Power className="w-2.5 h-2.5 stroke-[3.5]" />
            </div>
        ...
        <p className="text-xs text-gray-400 leading-relaxed">
          {translateText(
            "When active, users starting the bot must solve an interactive Persian math or emoji puzzle to verify they are human, preventing automated bots and spam abuse.",
            "وقتی این گزینه فعال باشد، کاربران جدید هنگام ورود به ربات باید یک تست ضد ربات (کپچا هوشمند) را حل کنند تا هویت انسانی آنها تایید شده و از ورود ربات‌های فیک، اسپم و سوءاستفاده از سیستم رفرال جلوگیری شود.",
            lang
          )}
        </p>

        {captchaActive ? (
          <div className="p-3 bg-emerald-950/20 border border-emerald-500/20 rounded-lg text-xs text-emerald-300 flex items-center gap-2 animate-fadeIn">
            <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse shrink-0"></span>
            <span>
              {translateText(
                "Captcha system is ACTIVE. Users must pass the challenge before accessing bot features.",
                "سیستم کپچا فعال است. کاربران قبل از دسترسی به امکانات ربات باید تست ضد ربات را با موفقیت پاس کنند.",
                lang
              )}
            </span>
          </div>
        ) : (
          <div className="p-3 bg-slate-800/40 border border-slate-700/50 rounded-lg text-xs text-slate-400 flex items-center gap-2">
            <span className="w-2 h-2 rounded-full bg-slate-500 shrink-0"></span>
            <span>
              {translateText(
                "Captcha is DISABLED. Users can directly interact with the bot without verification.",
                "سیستم کپچا غیرفعال است. کاربران بدون نیاز به حل تست امنیتی مستقیماً وارد ربات می‌شوند.",
                lang
              )}
            </span>
          </div>
        ...
        {/* Save button specific to Captcha */}
        <div className="flex justify-end pt-1">
          <button
            type="button"
            onClick={(e) => handleSubmit(e)}
            className="px-4 py-2 text-xs font-semibold rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white transition cursor-pointer flex items-center gap-1.5"
          >
            {translateText("Save Captcha Settings", "ذخیره تنظیمات کپچا", lang)}
          </button>
        </div>
      </div>
