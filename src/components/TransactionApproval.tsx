...
export default function TransactionApproval({
  ...
  // Helper to determine whether a transaction is a deposit receipt / card-to-card slip
  const isReceiptDepositTx = (tx: Transaction) => {
    // 1. Has uploaded receipt image
    if (tx.receiptImage && tx.receiptImage.trim() !== "" && tx.receiptImage !== "null") return true;
    
    // 2. Explicit card / deposit transaction identifiers or types
    const id = String(tx.id || "");
    if (id.startsWith("tx_") || id.startsWith("TX-DEP-") || id.startsWith("TX-CARD-")) return true;
    if (tx.type === "deposit" || tx.type === "card" || tx.type === "card_to_card" || tx.paymentMethod === "card_to_card" || tx.paymentMethod === "card") return true;
    
    // 3. Exclude internal wallet balance deductions (e.g. TX-PUR-..., TX-COL-...) that do not have receipt images
    if (id.startsWith("TX-PUR-") || id.startsWith("TX-COL-") || tx.type === "purchase") return false;
    
    return false;
  };

  // Only display actual deposit transactions (receipt uploads / card-to-card deposits)
  const depositTransactions = transactions.filter(isReceiptDepositTx);

  const filteredTransactions = depositTransactions
    .filter(tx => {
      if (filterStatus !== "all") {
  ...
  const statusCounts = {
    all: depositTransactions.length,
    pending: depositTransactions.filter(t => getNormalizedStatus(t.status) === "pending").length,
    approved: depositTransactions.filter(t => getNormalizedStatus(t.status) === "approved").length,
    rejected: depositTransactions.filter(t => ["rejected", "refunded", "failed"].includes(getNormalizedStatus(t.status))).length,
  };
