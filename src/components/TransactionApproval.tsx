...
export default function TransactionApproval({
  ...
  const checkIsRealImage = (img?: string) => {
    if (!img) return false;
    const s = img.trim();
    if (s.startsWith("bg-")) return false;
    if (
      s.startsWith("data:") || 
      s.startsWith("http") || 
      s.startsWith("/") || 
    ...
    <div id="transactions-tab" className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-[#111827] border border-[#1f2937] p-5 rounded-xl flex flex-col justify-between h-fit space-y-4">
          <div className="space-y-4">
            ...
            {selectedTx ? (
              (() => {
                const isRealImage = checkIsRealImage(selectedTx.receiptImage);
                const imgSrc = getImgSrc(selectedTx.receiptImage);
                return (
      ...
      {lightboxOpen && selectedTx && (
        ...
        >
          ...
          >
            <img 
              src={getImgSrc(selectedTx.receiptImage)} 
