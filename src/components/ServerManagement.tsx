...
export default function ServerManagement({
  ...
  const [deletingPricingBoxId, setDeletingPricingBoxId] = useState<string | null>(null);
    ...
    <div className="space-y-6">
      ...
      <div className="bg-[#111827] border border-[#1f2937] p-5 rounded-2xl space-y-4 shadow-sm">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 h-[38px]">
              <button
                ...
                style={{ direction: "ltr" }}
                title={localIsFreeTestActive ? translateText("Enabled", "فعال", lang) : translateText("Disabled", "غیرفعال", lang)}
              >
                <div
                  ...
                  style={{
                    left: localIsFreeTestActive ? "22px" : "2px",
                    top: "2px",
                    color: localIsFreeTestActive ? "#059669" : "#94a3b8",
                  }}
                >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-[10px] text-gray-400 uppercase mb-1.5 font-bold">
              {translateText("Free Test Duration (Days)", "مدت زمان اکانت تست (روز)", lang)}
            </label>
            <input
              type="text"
      ...
      <div className="bg-[#111827] border border-[#1f2937] p-5 rounded-2xl space-y-4 shadow-sm">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-gray-800 pb-3">
          <div className="flex items-center gap-3">
            ...
            <button
              type="button"
              onClick={() => {
                const nextActive = settings.isCustomPricingActive !== false ? false : true;
                onSaveSettings({
                  ...settings,
                  isCustomPricingActive: nextActive
                });
              }}
              className={`relative inline-flex h-5 w-10 shrink-0 cursor-pointer rounded-full transition-colors duration-300 focus:outline-none ${
              ...
              style={{ direction: "ltr" }}
              title={settings.isCustomPricingActive !== false ? translateText("ON", "روشن", lang) : translateText("OFF", "خاموش", lang)}
            >
              <div
                className="absolute flex items-center justify-center h-4 w-4 rounded-full bg-white transition-all duration-300 ease-in-out"
                style={{
                  left: settings.isCustomPricingActive !== false ? "22px" : "2px",
                  top: "2px",
                  color: settings.isCustomPricingActive !== false ? "#059669" : "#94a3b8",
                }}
              >
                <Power className="w-2.5 h-2.5 stroke-[3.5]" />
              </div>
        ...
        {settings.isCustomPricingActive !== false ? (
            ...
            {pricingBoxes.length === 0 ? (
              <div className="text-center py-8 bg-slate-950/40 rounded-xl border border-dashed border-gray-800">
                <p className="text-xs text-gray-500">
                  {translateText("No rules defined. Bot will use default prices: 3,000 per GB and 2,000 per Day.", "هیچ قانون قیمت‌گذاری تعریف نشده است. (ربات از مقادیر پیش‌فرض استفاده خواهد کرد: هر گیگ ۳,۰۰۰ و هر روز ۲,۰۰0 تومان)", lang)}
                </p>
              </div>
            ) : (
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            ...
                            <div>
                              ...
                              <input
                                ...
                                className="w-full bg-[#111827] border border-gray-750 rounded-lg p-2 text-xs text-white focus:border-indigo-500 outline-none font-mono"
                                placeholder="1"
                              />
                            </div>
                            <div>
          ...
          <div className="text-center py-6 bg-rose-500/5 rounded-xl border border-dashed border-rose-500/30 p-4 space-y-2">
            <p className="text-xs text-rose-400 font-bold flex items-center justify-center gap-1.5">
              <span>⚠️</span>
              <span>
                {translateText("Custom pricing feature is currently OFF.", "تنظیمات قیمت‌گذاری و خرید حجم دلخواه غیرفعال است.", lang)}
              </span>
            </p>
            <p className="text-xs text-amber-300 font-medium">
              {translateText("Notice: When this setting is OFF, the 'Create Custom Volume Config' button is completely hidden and disabled in the Telegram bot.", "هشدار: با خاموش بودن این گزینه، دکمه «ساخت کانفیگ با حجم دلخواه» به طور کامل از ربات تلگرام ناپدید و غیرفعال می‌شود.", lang)}
            </p>
          </div>
        )}
      </div>
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    className="p-1.5 bg-rose-500/10 text-rose-400 rounded-lg hover:bg-rose-500/20 transition-colors cursor-pointer"
                    title={translateText("Delete", "حذف دسته‌بندی", lang)}
                  >
      ...
      <ConfirmationModal
        ...
        message={
          deletingCatId !== null
            ? translateText(
                ...
                `آیا از حذف دسته‌بندی «${planCategories.find(c => c.id === deletingCatId)?.name || deletingCatId}» اطمینان کامل دارید؟`,
                lang
              )
            : ""
        }
        onConfirm={() => {
          if (deletingCatId !== null) {
            handleDeleteCategory(deletingCatId);
        ...
        lang={lang}
        isDangerous={true}
        confirmText={translateText("Delete Category", "بله، حذف دسته‌بندی", lang)}
        cancelText={translateText("Cancel", "انصراف", lang)}
      />

      {/* Confirmation Modal for Pricing Box Deletion */}
      <ConfirmationModal
        isOpen={deletingPricingBoxId !== null}
        message={
          translateText(
        ...
        onConfirm={() => {
          if (deletingPricingBoxId !== null) {
            handleDeletePricingBox(deletingPricingBoxId);
            setDeletingPricingBoxId(null);
          }
        }}
