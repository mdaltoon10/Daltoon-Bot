...
export default function DashboardOverview({
  ...
  const [backupModalMsg, setBackupModalMsg] = useState<string | null>(null);
  ...
  // Live Advanced Financial Calculations (Exclusively verified card-to-card receipts & direct deposits)
  const approvedTxs = (transactions || []).filter(tx => isCardOrDepositTransaction(tx));
  ...
  return (
    <div id="dashboard-tab" className="space-y-6">
                  ...
                  reader.onload = async (ev) => {
                     ...
                     if (typeof content === "string") {
                            try {
                              ...
                              const resText = await fb.text();
                              let rJson: any;
                              try {
                                rJson = JSON.parse(resText);
                              } catch (pErr) {
                                throw new Error(`پاسخ سرور (${fb.status}): ${resText.slice(0, 150) || "پاسخ خالی است"}`);
                              }
                              if (rJson.success) {
                                console.log(dt.backupSuccess);
                                setBackupModalMsg(dt.backupSuccess);
                                setTimeout(() => window.location.reload(), 1500);
                              } else {
                                console.error(rJson.error || "Error restoring backup");
                                setBackupModalMsg(rJson.error || "Error restoring backup");
                              }
                            } catch(er: any) {
                              console.error(er.message);
                              setBackupModalMsg("خطا در ارسال بکاپ: " + er.message);
                            }
                     }
                  };
                  if (isJson) {
      ...
      <ConfirmationModal
        isOpen={!!backupModalMsg}
        title={lang === "fa" ? "بازیابی بکاپ" : "Restore Backup"}
        message={backupModalMsg || ""}
        lang={lang}
        isAlert={true}
        confirmText={lang === "fa" ? "متوجه شدم" : "OK"}
        onConfirm={() => setBackupModalMsg(null)}
      />
    </div>
  );
}
