...
export default function UserManagement({
  ...
  const [filterTab, setFilterTab] = useState<"all" | "active" | "has_service" | "has_balance" | "banned" | "verified">("all");
  ...
  return (
    <div id="users-tab" className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        ...
        >
          <span className="text-xs font-medium text-cyan-400/90">{translateText("Verified Users", "احراز هویت شده", lang)}</span>
          <div className="flex items-center justify-between mt-2">
            <span className="w-2.5 h-2.5 rounded-full bg-cyan-500 shadow-[0_0_8px_rgba(6,182,212,0.8)] animate-pulse"></span>
            <span className="text-lg md:text-xl font-bold font-mono text-cyan-400">{users.filter(u => u.isHumanVerified || u.captchaPassed).length}</span>
          </div>
        </button>
      </div>
                    ...
                    <tr className={`transition ${isExpanded ? 'bg-slate-900/60' : 'hover:bg-slate-900/40'}`}>
                      <td className="px-5 py-4">
                        <div className="flex flex-col gap-2">
                          ...
                          <div className="flex items-center gap-1 font-medium text-white flex-wrap">
                            <span className="text-indigo-400">@</span>
                            <span>{user.username}</span>
                            {(user.isHumanVerified || user.captchaPassed) && (
                              <span className="ml-1 bg-cyan-500/10 text-cyan-400 border border-cyan-500/20 px-1.5 py-0.5 rounded text-[10px] font-bold">
                                {translateText("Verified", "احراز هویت شده", lang)}
                              </span>
                            )}
                            <button
                              onClick={() => {
