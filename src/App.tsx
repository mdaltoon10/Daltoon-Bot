...
import { isCardOrDepositTransaction } from "./utils/revenueUtils";
...
export default function App() {
  ...
  const isDemoEnv = window.location.hostname.includes("ais-dev-") || window.location.hostname.includes("ais-pre-");
  ...
  const [isAuthenticated, setIsAuthenticated] = useState<boolean>(() => {
    ...
    // Auto-logout if more than 24 hours passed since last interaction or token is missing
    if ((isAuth && Date.now() - lastInteraction > 86400000) || (isAuth && !token)) {
  ...
  useEffect(() => {
    if (isAuthenticated) {
      ...
      const hasValidOwner = settings.ownerId && Number(settings.ownerId) > 0;

      // Force setup modal if explicitly a new install or if botToken / ownerId is not yet configured
      if (!isDemoEnv && (isNewInstall === true || !hasValidToken || !hasValidOwner)) {
        setShowSetupModal(true);
      }
    }
  }, [isAuthenticated, isNewInstall, settings.botToken, settings.ownerId]);
  ...
  const handleReplyTicket = (ticketId: string, replyMessage: string) => {
    // ⚡ Optimistic UI update: instantly append admin reply to the active chat
    const newMsg = {
  ...
  // Metrics calculators (Revenue is exclusively calculated from approved card-to-card receipts & direct deposits)
  const pendingTx = transactions.filter((t) => t.status === "pending");
  const totalVolume = transactions
    .filter((t) => isCardOrDepositTransaction(t))
