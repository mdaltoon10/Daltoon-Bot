...
async function run() {
  ...
  try {
    ...
    try {
      execSync(`git commit -m "release: v${VERSION} - fix unresponsive bot and clear PM2 zombies"`);
    } catch {
...
- **PM2 Zombie Process & Unresponsive Bot Fix**:
  - Implemented an aggressive \`pkill -f 'python3.*bot.py'\` cleanup before any PM2 restart. This wipes out orphaned 'zombie' bot processes that were created by previous Node.js \`spawn\` leaks, which silently hijacked the bot token and blocked PM2 from receiving \`/start\` commands.
  - Added an intelligent fallback: if \`daltoon-bot\` does not exist in the PM2 list (e.g. for users updating from very old versions), the web server will seamlessly register and start it in PM2 via \`pm2 start\` rather than failing silently.
- **Includes all previous Auto-Backup Interval Fixes**:
  - Properly respects hourly/daily/weekly auto-backup schedules.`,
      draft: false,
      prerelease: false,
    };
