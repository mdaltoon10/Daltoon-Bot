...
async function run() {
  ...
  try {
    ...
    try {
      execSync(`git commit -m "release: v${VERSION} - fix auto-backup interval persistence & PM2 bot lifecycle"`);
    } catch {
...
- **Auto-Backup Interval & Persistence Fix**:
  - Resolved an issue where auto-backup would send every minute due to failed partial database writes.
  - Ensured \`lastAutoBackup\` timestamps are permanently saved and respected according to the configured interval (hourly, daily, weekly, monthly).
  - First-time startup initializes the timer schedule smoothly without triggering immediate repetitive backups.
- **Python Telegram Bot Process Lifecycle**:
  - Enhanced PM2 integration with full environment PATH support for starting, restarting, and checking bot status.
  - Added robust fallback to internal spawn if PM2 is not managing the bot process.
- **Security & Authorization**:
  - Full cryptographic HMAC-SHA256 session tokens and centralized API route authorization.`,
      draft: false,
      prerelease: false,
    };
