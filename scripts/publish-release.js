...
- **Per-Server Payment Methods Management**:
  - Added modern per-server allowed payment methods controls in X-UI server management settings.
  - Granular toggles for Wallet, Card-to-Card, Plisio, NowPayments, Cryptomus, Heleket, and Telegram Stars.
  - Live active payment methods badges inside the server management table.
- **Dynamic Purchase Flow Filtering**:
  - MiniApp purchase screens dynamically hide and adapt payment method buttons based on the chosen server.
  - Telegram bot automatically filters out disallowed payment methods during regular purchase, custom plan creation, and renewal flows.
- **Purchase & Renewal Notifications**:
  - Real-time display of the selected payment method (Wallet, Card-to-Card, Telegram Stars, Free Test, etc.) in all admin purchase alerts.
- **Discount & Referral System Improvements**:
  - Active/Inactive status controls for discount codes.
  - Enhanced negative balance handling on referral refund/exits.`,
      draft: false,
      prerelease: false,
    };
