...
async function run() {
  ...
  try {
    ...
    try {
      execSync(`git commit -m "release: v${VERSION} - fix PM2 duplicate bot process conflict (final fix)"`);
    } catch {
...
- **PM2 Detection Path Final Fix**:
  - Injected explicit Node.js NVM execution paths directly into the \`exec\` environment of the web server. This ensures \`pm2 jlist\` and \`pm2 restart\` commands are always successfully found and executed by the backend, regardless of standard Linux PATH constraints.
  - **Hardened PM2 Safeguard**: Prevented the server from ever falling back to internal bot spawning when PM2 environment variables (\`PM2_HOME\`) are present. This entirely neutralizes the 409 Conflict loop.
