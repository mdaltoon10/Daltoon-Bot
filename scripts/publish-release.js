...
async function run() {
  ...
  try {
    ...
    try {
      execSync(`git commit -m "release: v${VERSION} - per-server payment methods control and purchase notifications update"`);
    } catch {
...
- **Promo Code Server Selection Fix**:
  - Strictly limited server selection in Promo / Discount Code management to standard servers from Server Management (\`settings.servers\`).
  - Removed unwanted fallback to individual inbounds and colleague servers in discount code configuration.
  - Added clean flag and server name badges to the server selection interface.
- **Wallet & Referral Persistence Safeguard**:
  - Enhanced multi-field user balance synchronization (\`walletBalance\`, \`wallet_balance\`, \`balance\`, \`credit\`) ensuring referral rewards and wallet funds are permanently preserved and never reset on startup or re-registration.
  - Upgraded user resolution across all legacy and current identifier schemes.
- **Admin Transaction Approval & Delivery Notifications**:
  - Explicit confirmation messages and alerts when approving receipts, confirming that configs have been successfully built and delivered to the buyer.
  - Real-time comprehensive delivery reports dispatched directly to the admin Telegram PV upon transaction confirmation.
- **Permanent Database Protection on Updates**:
  - Prioritized SQLite database (\`Daltoon_Bot.db\`) in \`install.sh\` installer and dashboard auto-updater.
  - Automated permanent archiving of database snapshots to \`/var/backups/daltoon\` before updates.`,
      draft: false,
      prerelease: false,
    };
