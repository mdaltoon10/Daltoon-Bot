...
async function run() {
  ...
  try {
    ...
    try {
      execSync(`git commit -m "release: v${VERSION} - fix domain name panel connections and TLS/SSL dispatcher resilience"`);
    } catch {
  ...
  console.log(`[Step 6/8] Creating or updating GitHub release for v${VERSION}...`);
  let releaseId = "";
  try {
    const payload = {
      tag_name: `v${VERSION}`,
...
- **Domain & Dashboard Data Fetch Fix**:
  - Made the update checking endpoint (\`/api/system/check-update\`) and public routes accessible seamlessly without unauthorized (401) session resets.
  - Enhanced global fetch interceptor in \`src/main.tsx\` to preserve token sessions across custom domain names and bypass spurious invalidations.
  - Persisted authentication secret key directly in SQLite KV store for resilient restart survival.
- **Client & Backend Synchronization**:
  - Synchronized client version identifier to match backend version (\`v${VERSION}\`).
  - Improved layout hierarchy: Bot Settings & Ticket Management ordering.
- **TLS/SSL & Panel Connection Resilience**:
  - Maintained global undici dispatcher and dual-scheme HTTP/HTTPS fallbacks for panel connections.`,
      draft: false,
      prerelease: false,
    };
    ...
    if (res.ok) {
      ...
      console.log("Release might already exist on tag, checking existing release...");
      const getTagRes = await fetch(`https://api.github.com/repos/${REPO}/releases/tags/v${VERSION}`, { headers });
      if (getTagRes.ok) {
        const existingData = await getTagRes.json();
        releaseId = existingData.id;
        console.log(`Found existing release (ID: ${releaseId}). Updating release body and assets...`);
        ...
        if (patchRes.ok) {
          console.log(`✅ GitHub release ${releaseId} updated successfully.`);
        }
        // Delete old assets to avoid upload conflicts
        if (existingData.assets && existingData.assets.length > 0) {
          for (const asset of existingData.assets) {
    ...
    console.error("❌ Error creating/updating release:", err.message);
    process.exit(1);
  }
