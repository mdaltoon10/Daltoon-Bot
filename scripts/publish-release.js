...
Welcome to the **v${VERSION} major release** of Daltoon Bot & Dashboard!

#### Major Additions & Stability Fixes in v${VERSION}:
- **Resolved Bot Crash on Startup (\`NameError: bot\`):**
  - Successfully fixed a critical \`NameError: name 'bot' is not defined\` bug on start by properly initializing the global \`bot\` object at the module level.
- **Resolved Crash Loop (\`RecursionError\`):**
  - Fixed a stack overflow recursion bug inside the \`safe_html_escape\` utility where the helper infinitely called itself. It now calls the standard Python \`html.escape\` method correctly.
- **Bulletproof Telegram HTML Parse Error Prevention:**
  - Enhanced custom message templating so HTML parsing failures automatically fallback gracefully to plain text to ensure the bot remains 100% online under any admin customization.
- **Optimized Server & Bot Performance:**
  - Streamlined PM2 task structures and database synchronizations for high-concurrency environments.`,
      draft: false,
      prerelease: false,
    };
