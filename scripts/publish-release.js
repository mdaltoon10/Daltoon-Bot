...
async function run() {
  ...
  try {
    ...
    try {
      execSync(`git commit -m "release: v${VERSION} - security hardening, cryptographic HMAC admin session tokens and API authorization middleware"`);
    } catch {
...
#### Key Improvements & Security Updates in v${VERSION}:
- **Cryptographic Admin Session Tokens (HMAC-SHA256)**:
  - Upgraded admin login authentication to produce tamper-proof, cryptographically signed HMAC-SHA256 tokens linked to persistent server-side secret keys.
  - Mitigated client-side response tampering and forged login state exploits.
- **Strict API Authorization Middleware (\`requireAdminAuth\`)**:
  - Enforced central, impenetrable authentication across all administrative backend routes (\`/api/data\`, bot configs, panel settings, transactions, user management, backups, and database operations).
  - Unauthorized requests are strictly rejected with \`401 Unauthorized\`.
- **Hybrid Token & Cookie Transmission**:
  - Implemented dual-channel authentication using both \`Authorization: Bearer <token>\` and \`daltoon_session\` cookies ensuring seamless cross-domain and reverse-proxy compatibility.
- **Session Auto-Expiry & Secure Invalidation**:
  - Active session verification with automated cleanup on logout or inactivity.`,
      draft: false,
      prerelease: false,
    };
