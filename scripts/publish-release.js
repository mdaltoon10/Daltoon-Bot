...
- **Bulletproof /start & Telegram HTML Parse Error Prevention**:
  - Resolved an issue where the bot could become unresponsive if there were unescaped HTML characters or parsing errors in the admin's custom text templates (e.g. \`BOT_NICKNAME\`, \`WELCOME_TEXT\`, \`SUPPORT_TEXT\`, \`MANDATORY_JOIN_TEXT\`).
  - Implemented a custom \`safe_html_escape\` wrapper ensuring any null or empty config variables are safely formatted as empty strings.
  - Added try-except fallback mechanisms for message delivery. If sending a formatted HTML message fails due to tag errors in the custom welcome or channel join warning, the bot automatically strips the tags and delivers the text as plain text. This ensures the bot remains 100% responsive and active for all users under any config.
- **Dashboard & Bot Synchronization Fix**:
  - Fixed an edge-case where the bot and dashboard read from different SQLite database files if run from a non-standard directory. PM2 now strictly launches the Python bot with the exact same CWD as the web dashboard.
- **Ultimate PM2 Cleanup**:
  - Expanded the zombie process killer (\`pkill -f 'bot.py'\`) to ensure absolutely no duplicate or stale Python processes can remain running in the background and hijack the Telegram token.`,
      draft: false,
      prerelease: false,
    };
