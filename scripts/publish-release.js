...
async function run() {
  ...
  try {
    ...
    try {
      execSync(`git commit -m "release: v${VERSION} - fix PM2 duplicate bot process conflict"`);
    } catch {
...
- **PM2 Duplicate Bot Process Fix**:
  - Reverted aggressive \`pm2 jlist\` PATH modifications that caused PM2 environments to silently fail detection.
  - Fixed a critical conflict where PM2 was running \`daltoon-bot\` but the server couldn't detect it, causing it to spawn a duplicate internal bot process.
  - This resolves the \`409 Conflict\` errors where the bot stopped responding to Telegram messages due to two instances fighting for the token.
