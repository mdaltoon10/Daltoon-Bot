...
async function run() {
  ...
  try {
    ...
    try {
      execSync(`git commit -m "release: v${VERSION} - fix Dashboard Sync and HTML parsing bugs"`);
    } catch {
...
- **Dashboard & Bot Synchronization Fix**:
  - Fixed a critical edge-case where the bot and dashboard could end up reading from two different SQLite databases if installed in a non-standard directory, causing the dashboard to appear disconnected from the bot. PM2 now strictly forces the Python bot to launch with the exact same Current Working Directory (CWD) as the web dashboard.
- **Telegram HTML Parse Crash Fix**:
  - Escaped all incoming HTML characters (like \`<\` or \`>\`) in user Telegram names and custom welcome messages. Previously, if a user had a \`<\` in their Telegram name or if the admin's welcome text contained invalid HTML, it would crash the \`send_message\` API silently, leaving the user with no reply to \`/start\`.
- **Ultimate PM2 Cleanup**:
  - Expanded the zombie process killer (\`pkill -f 'bot.py'\`) to ensure absolutely no ghost Python bots can remain running in the background and hijack the telegram token.`,
      draft: false,
      prerelease: false,
    };
