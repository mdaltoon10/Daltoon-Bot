...
async function run() {
  ...
  // 1. Delete releases v2.4.2, v2.4.2, v2.4.2 if they exist
  ...
  try {
    execSync("git tag -d v2.4.2", { stdio: "inherit" });
  } catch (err) {
    console.log("Some local tags did not exist or failed to delete locally.");
  }

  try {
    execSync(`git push origin :refs/tags/v2.4.2`, { stdio: "inherit" });
  ...
  try {
    ...
    execSync('git commit -m "release: v2.4.2 - major fix for update system and process coordination" || echo "No changes to commit"', { stdio: "inherit" });
  ...
  // 4. Create and push the v2.4.2 tag
  console.log("Creating and pushing local v2.4.2 tag...");
  try {
    execSync("git tag v2.4.2", { stdio: "inherit" });
    execSync("git push origin v2.4.2", { stdio: "inherit" });
    console.log("Tag v2.4.2 pushed successfully.");
  } catch (err) {
  ...
  console.log("Creating new GitHub release for v2.4.2...");
  let newReleaseId = "";
  try {
    const payload = {
      tag_name: "v2.4.2",
