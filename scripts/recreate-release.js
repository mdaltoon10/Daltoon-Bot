...
async function run() {
  ...
  // 1. Delete releases v2.3.3, v2.3.2, v2.3.2 if they exist
  ...
  const tagsToDelete = ["v2.3.2", "v2.3.2", "v2.3.3"];
  ...
  try {
    execSync("git tag -d v2.3.2 v2.3.2 v2.3.3", { stdio: "inherit" });
  } catch (err) {
    console.log("Some local tags did not exist or failed to delete locally.");
  }

  try {
    execSync(`git push origin :refs/tags/v2.3.2 :refs/tags/v2.3.2 :refs/tags/v2.3.3`, { stdio: "inherit" });
  ...
  try {
    ...
    execSync('git commit --amend -m "release: v2.3.2 - complete multi-language, currency support, and cleanups" || git commit -m "release: v2.3.2 - complete multi-language, currency support, and cleanups" || echo "No changes to commit"', { stdio: "inherit" });
  ...
  // 4. Create and push the v2.3.2 tag
  console.log("Creating and pushing local v2.3.2 tag...");
  try {
    execSync("git tag v2.3.2", { stdio: "inherit" });
    execSync("git push origin v2.3.2", { stdio: "inherit" });
    console.log("Tag v2.3.2 pushed successfully.");
  } catch (err) {
  ...
  console.log("Creating new GitHub release for v2.3.2...");
  let newReleaseId = "";
  try {
    const payload = {
      tag_name: "v2.3.2",
      ...
      name: "v2.3.2",
      body: "### Changes in v2.3.2\n\n- Added multi-language support (English, Persian, Arabic, Russian, Turkish, Spanish)\n- Added customizable currency setting properly handled everywhere\n- Enabled auto-generation of 5-7 alphanumeric suffixes for newly created usernames to prevent name collision\n- Updated documentation and README with multi-language localizations\n- General code cleanups and stability updates",
      draft: false,
      prerelease: false,
    };
