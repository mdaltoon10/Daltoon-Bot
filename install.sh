...
PERMANENT_BACKUP_DIR="/var/backups/daltoon"
mkdir -p "$BACKUP_DIR"
...
CANDIDATE_DIRS=("/opt/daltoon-store" "$(pwd)" "$HOME" "/root" "/root/daltoon" "/root/DaltoonBot" "/root/daltoon-store" "/root/daltoon-bot" "/root/daltoonbot" "/root/Daltoon_Bot" "/opt/daltoon" "/opt/daltoon-bot" "/home/daltoon" "/var/www/daltoon" "/var/daltoon" "$PERMANENT_BACKUP_DIR")

# Collect all files that exist in candidates
FOUND_FILES=()
for dir in "${CANDIDATE_DIRS[@]}"; do
    if [ -d "$dir" ]; then
        for f in "Daltoon_Bot.db" "Daltoon_Bot.db-wal" "Daltoon_Bot.json" "database.json" "db.json" "bot_database.json" ".env"; do
...
ADDITIONAL_FIND=$(find /root /home /opt /var -maxdepth 4 -type f \( -name "Daltoon_Bot.db" -o -name "Daltoon_Bot.json" -o -name "database.json" -o -name "db.json" -o -name "bot_database.json" -o -name ".env" \) 2>/dev/null | grep -vE "/proc|/sys|/dev|/var/lib/docker|/snap|/tmp|/run|/node_modules|/\.git")
...
const PERMANENT_BACKUP_DIR = '$PERMANENT_BACKUP_DIR';
const filesStr = process.env.UNIQUE_FILES || '';
const files = filesStr.split('\n').map(f => f.trim()).filter(Boolean);

let bestSqliteDb = null;
let bestSqliteScore = -1;
...
function getSqliteScore(p) {
  try {
    if (!fs.existsSync(p)) return -1;
    const stat = fs.statSync(p);
    if (stat.size < 100) return -1;
    
    // Quick python probe to count users in SQLite kv table
    const py = 'import sqlite3, json; conn=sqlite3.connect(\"' + p.replace(/\\\\/g, '/') + '\"); c=conn.cursor(); c.execute(\"SELECT value FROM kv WHERE key=\\'users\\'\"); r=c.fetchone(); conn.close(); u=json.loads(r[0]) if r else []; print(len(u))';
    try {
      const out = execSync('python3 -c ' + JSON.stringify(py), { encoding: 'utf8', timeout: 5000 }).trim();
      const count = parseInt(out, 10) || 0;
      return (count * 10000) + stat.size;
    } catch(e) {
      return stat.size;
    }
  } catch(e) { return -1; }
...
for (const p of files) {
  ...
  } else if (baseName === 'Daltoon_Bot.db') {
    const score = getSqliteScore(p);
    if (score > bestSqliteScore) {
      bestSqliteScore = score;
      bestSqliteDb = p;
    }
  } else {
    const score = getJsonScore(p);
...
if (bestSqliteDb) {
  console.log('\x1b[32m[Installer Backup] Saving active SQLite database from ' + bestSqliteDb + ' (Score: ' + bestSqliteScore + ')\x1b[0m');
  fs.copyFileSync(bestSqliteDb, path.join(BACKUP_DIR, 'Daltoon_Bot.db'));
  try {
    const wal = bestSqliteDb + '-wal';
    if (fs.existsSync(wal)) fs.copyFileSync(wal, path.join(BACKUP_DIR, 'Daltoon_Bot.db-wal'));
    // Permanent archive
    const timestamp = Date.now();
    fs.copyFileSync(bestSqliteDb, path.join(PERMANENT_BACKUP_DIR, 'Daltoon_Bot_' + timestamp + '.db'));
  } catch(e) {}
}

if (bestJsonDb) {
  console.log('\x1b[32m[Installer Backup] Saving legacy database from ' + bestJsonDb + ' (Score: ' + bestJsonScore + ')\x1b[0m');
  fs.copyFileSync(bestJsonDb, path.join(BACKUP_DIR, 'Daltoon_Bot.json'));
}
"
...
// 1. Direct restore of SQLite Daltoon_Bot.db (Highest priority)
const sqliteBackup = path.join(backupDir, 'Daltoon_Bot.db');
if (fs.existsSync(sqliteBackup)) {
  fs.copyFileSync(sqliteBackup, path.join(installDir, 'Daltoon_Bot.db'));
  if (fs.existsSync(optDir)) {
    try { fs.copyFileSync(sqliteBackup, path.join(optDir, 'Daltoon_Bot.db')); } catch(e) {}
  }
  const walBackup = path.join(backupDir, 'Daltoon_Bot.db-wal');
  if (fs.existsSync(walBackup)) {
    fs.copyFileSync(walBackup, path.join(installDir, 'Daltoon_Bot.db-wal'));
    if (fs.existsSync(optDir)) {
      try { fs.copyFileSync(walBackup, path.join(optDir, 'Daltoon_Bot.db-wal')); } catch(e) {}
    }
  }
  console.log('\x1b[32m[Installer] Restored active SQLite Daltoon_Bot.db successfully.\x1b[0m');
}

// 2. Check if we have Daltoon_Bot.json in backup
let targetData = null;
...
// 3. If no valid Daltoon_Bot.json found in backup, try to find the best legacy backup file
if (!targetData) {
  let bestFile = null;
  let bestScore = -1;
...
// 4. Write unified data to both current directory and opt directory ONLY if SQLite db is missing
if (targetData && !fs.existsSync(path.join(installDir, 'Daltoon_Bot.db'))) {
...
if [ "$HAS_SSL" == "1" ]; then
    ...
    echo -e "${BLUE}              SSL Certificate Setup                 ${NC}"
    echo -e "${BLUE}====================================================${NC}"
    echo -e "1. Without SSL Certificate"
