...
// Add imports
code = code.replace(
  'import { Users, Server, Wallet, ShieldCheck, HardDrive, Cpu, Activity, Zap, Cloud, Send, TrendingUp, Star, Gift, TrendingDown, ArrowUpRight, ArrowDownRight } from "lucide-react";',
  'import { Users, Server, Wallet, ShieldCheck, HardDrive, Cpu, Activity, Zap, Cloud, Send, TrendingUp, Star, Gift, TrendingDown, ArrowUpRight, ArrowDownRight, Power, RotateCw } from "lucide-react";'
);
...
code = code.replace(
  ...
  'const { translateText } = useTranslations();' + stateInjection
);

// Replace the Daltoon Bot Card with the new layout
...
const newCard = `      {/* Daltoon Bot Card - Standalone Horizontal on Desktop */}
      <div className="bg-[#111827] border border-[#1f2937] p-5 rounded-xl flex flex-col gap-4">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          {/* PM2 Server Controls */}
          <div className="flex items-center gap-2 border border-[#1f2937] rounded-lg p-1 bg-[#0b101d]">
            <button
            ...
            >
              <RotateCw className={\`w-4 h-4 \${botActionLoading ? 'animate-spin' : ''}\`} />
            </button>
          </div>
        </div>

        <div className="w-full h-px bg-[#1f2937] my-1" />
