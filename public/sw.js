const CACHE_NAME = 'daltoon-pwa-v4-8-5';
...
self.addEventListener('fetch', (event) => {
  ...
  if (event.request.mode === 'navigate' || event.request.destination === 'document' || url.pathname === '/' || url.pathname === '/index.html' || url.pathname.startsWith('/miniapp')) {
    event.respondWith(
      fetch(event.request)
